...
class LinFukushimaStrategy(BaseStrategy):
    ...
    *Journal of Optimization Theory and Applications*, 118(1), 81–116.
    ...
    def _build_cleanup_hessian(self):
        ...
        # Pre-allocated zero-padded Lagrangian buffer.  The cleanup NLP
        # has the same dimension minus the 2·n_c rows for G·H ≤ ε and
        # G+H ≥ ε; padding those slots with zeros recovers the strategy
        # Lagrangian.  Allocating the buffer once and writing the leading
        # slice in-place avoids a fresh np.concatenate per IPOPT
        # iteration of the cleanup solve.  The user's
        # ``lagrangian_hessian`` callable must treat its ``lam`` argument
        # as read-only (cyipopt's contract).
        n_lam_cu = p.n_eq + p.n_ineq + 2 * p.n_comp
        lam_full_buf = np.zeros(n_lam_cu + 2 * p.n_comp)
    ...
    def solve(self) -> MPCCResult:
        ...
        def make_iteration(eps, x, last_info, n_ipopt_iter, iter_time):
            ...
            if _mg is not None and len(_mg):
                ...
                _lam_GPH = _mg[_off + 3 * n_c   : _off + 4 * n_c]
        ...
        if mg_final is not None and len(mg_final):
            ...
            lam_GPH = mg_final[_off + 3 * n_c   : _off + 4 * n_c]
